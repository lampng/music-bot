...
return interaction?.reply({ content: "Tỷ lệ giới hạn.", ephemeral: true })
} else {
    ...
    function cmd_loader() {
        if (interaction?.type === InteractionType.ApplicationCommand) {
            ...
            if (!interaction?.member?.voice?.channelId) return interaction?.reply({ content: `🔴 Tham gia kênh Voice chat trước!!`, ephemeral: true }).catch(e => { })
            ...
            return interaction?.reply({ content: `🔴 Phải ở cùng một Voice chat!!`, ephemeral: true }).catch(e => { })
            }
            }
            }
            ...
            return interaction?.reply({ content: `▶️ Thiếu quyền: **${props?.permissions?.replace("0x0000000000000020", "QUẢN LÝ HỘI NGHỊ")?.replace("0x0000000000000800", "GỬI TIN NHẮNS") || "GỬI TIN NHẮN"}**`, ephemeral: true });
            }
            } catch (e) {
